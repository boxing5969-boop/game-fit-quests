...
-- 이 앱의 보안 모델 (한 줄 요약)
-- ---------------------------------------------------------------------
--   pt_* 테이블 11개 + dashboard_users 전부 RLS ON + **정책 0개**  ==>
...
--   PostgREST 로 pt_consults(개인정보)나 pt_webauthn_creds(자격증명)를
--   통째로 읽게 된다.
--
-- ---------------------------------------------------------------------
-- 2026-08-09 (3차) 이 파일에서 달라진 것
-- ---------------------------------------------------------------------
--   1. 신규 테이블 pt_webauthn_creds / pt_wa_challenges :
--      둘 다 RLS ON, 정책 0, anon/authenticated 권한 0, service_role 만 ALL.
--      (다른 pt_* 테이블과 동일 정책 -- 실측 확인)
--   2. 신규 함수 7개 pt_wa_challenge_new / pt_wa_challenge_take / pt_wa_register /
--      pt_wa_get / pt_wa_touch / pt_wa_list / pt_wa_del :
--      PUBLIC + anon + authenticated + service_role EXECUTE (Supabase 기본값 그대로).
--      방어선은 EXECUTE 권한이 아니라 함수 안의 _pt_admin_ok(p_secret) 검사다.
--   3. 신규 시퀀스 pt_webauthn_creds_id_seq -- 아래 (C-2) 참고. Supabase 기본값이라
--      anon 에 USAGE(w) 가 붙어 있다. 테이블 권한이 없어 실제로는 무해하지만
--      실측 그대로 기록해 둔다.
--   4. 그 밖의 RLS/정책/GRANT 는 2차와 **완전히 동일**하다 (1건도 안 바뀜).
-- =====================================================================
...
--   pt_tg_invites      | true           | false               | 0
--   pt_tg_recipients   | true           | false               | 0
--   pt_wa_challenges   | true           | false               | 0   ★신규
--   pt_webauthn_creds  | true           | false               | 0   ★신규
...
ALTER TABLE public.pt_wa_challenges  ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pt_webauthn_creds ENABLE ROW LEVEL SECURITY;
...
--   pt_tg_invites     | ALL      | (없음)    | (없음)        | ALL
--   pt_tg_recipients  | ALL      | (없음)    | (없음)        | ALL
--   pt_wa_challenges  | ALL      | (없음)    | (없음)        | ALL   ★신규
--   pt_webauthn_creds | ALL      | (없음)    | (없음)        | ALL   ★신규
...
GRANT ALL ON TABLE public.pt_wa_challenges  TO service_role;
GRANT ALL ON TABLE public.pt_webauthn_creds TO service_role;
...
REVOKE ALL ON TABLE public.pt_admin_secret,  public.pt_coaches,        public.pt_consults,
                    ...
                    public.pt_wa_challenges, public.pt_webauthn_creds
  FROM anon, authenticated;


-- ---------------------------------------------------------------------
-- (C-2) 시퀀스 GRANT -- 실측값 (참고)
-- ---------------------------------------------------------------------
--   시퀀스              | postgres | anon | authenticated | service_role
--   --------------------+----------+------+---------------+--------------
--   pt_coaches_id_seq       | rwU  | w    | rwU           | rwU
--   pt_consults_id_seq      | rwU  | w    | rwU           | rwU
--   pt_logs_id_seq          | rwU  | w    | rwU           | rwU
--   pt_members_id_seq       | rwU  | w    | rwU           | rwU
--   pt_tg_recipients_id_seq | rwU  | w    | rwU           | w
--   pt_webauthn_creds_id_seq| rwU  | w    | rwU           | rwU   ★신규
--
--   전부 Supabase 가 신규 시퀀스에 붙이는 기본값이며, 이 앱이 명시적으로
--   설정한 것이 아니다. 테이블 자체에 anon/authenticated INSERT 권한이 없으므로
--   시퀀스 USAGE 만으로는 아무것도 못 한다(무해). 그래도 조이고 싶다면:
--     -- REVOKE ALL ON SEQUENCE public.pt_webauthn_creds_id_seq FROM anon, authenticated;
--   service_role 에는 반드시 남겨둘 것 (SECURITY DEFINER 가 아닌 경로로 INSERT 시 필요).
GRANT USAGE, SELECT, UPDATE ON SEQUENCE public.pt_webauthn_creds_id_seq TO service_role;
...
-- ★ 2026-08-09 3차 신규: 지문·얼굴 로그인(WebAuthn 패스키) RPC 7개.
--   전부 anon/authenticated/service_role EXECUTE (운영 실측 그대로).
--   전부 첫 인자가 p_secret 이고 _pt_admin_ok 게이트를 통과해야 한다.
--   ※ 서버(main.ts /api/wa/*)에서만 호출할 것. 시크릿이 브라우저로 새면
--     아래 (E) 10번의 위험이 현실화된다.
GRANT EXECUTE ON FUNCTION public.pt_wa_challenge_new(text, text, text)                                                               TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.pt_wa_challenge_take(text, text, text)                                                              TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.pt_wa_register(text, text, text, text, jsonb, text)                                                 TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.pt_wa_get(text, text)                                                                               TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.pt_wa_touch(text, bigint, bigint)                                                                   TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.pt_wa_list(text, text)                                                                              TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.pt_wa_del(text, bigint, text)                                                                       TO anon, authenticated, service_role;
...
-- PUBLIC(=X) 엔트리에 대하여:
--   위 함수들은 아래만 빼고 전부 PUBLIC 에도 EXECUTE 가 있다 (Supabase 기본값).
...
--   pt_wa_* 7개는 PUBLIC EXECUTE 가 **있다**(기본값). 실측 확인.
...
--      이제 _bot_admin_ok(p_secret) 를 통과해야 한다.
...
-- 10. ★ 2026-08-09 신규 -- pt_wa_* 7개도 anon EXECUTE 가 열려 있고 방어선은
--      _pt_admin_ok(p_secret) 하나다. 시크릿이 브라우저로 새면
--        - pt_wa_register 로 **임의의 계정(user_key)에 공격자 기기를 등록**할 수 있고,
--        - pt_wa_challenge_take 로 남의 챌린지를 가로챌 수 있다.
--      비밀번호 로그인보다 오히려 더 치명적이다. ADMIN_SECRET 은 반드시
--      서버 환경변수로만 두고 클라이언트 번들에 넣지 말 것.
-- 11. pt_webauthn_creds 는 계정 테이블(pt_coaches / dashboard_users)과 **FK 가 없다**
--      (user_key 가 다형 문자열 키라서). 따라서 계정을 하드 DELETE 해도 등록 기기
--      행은 남는다. 로그인은 pt_wa_get 이 계정 상태를 재확인하므로 막히지만,
--      찌꺼기 행은 수동으로 지워야 한다:
