...
REVOKE ALL ON TABLE public.pt_admin_secret,  public.pt_coaches,   public.pt_consults,
                    ...
                    public.pt_sessions,      public.pt_tg_recipients
...
--
-- 내부 헬퍼 6개: anon/authenticated 에게 EXECUTE 를 주지 않는다 (중요).
--   _pt_admin_ok        : postgres 만
--   _pt_is_pt_product   : postgres 만
--   _pt_name_phone      : postgres 만
--   _pt_member_in_scope : postgres 만   (2026-08-08 신규)
--   _pt_phone_key       : postgres 만   (2026-08-08 신규. HMAC 키로 시크릿을 읽는다)
...
GRANT EXECUTE ON FUNCTION public.pt_member_attendance(text, bigint, integer)                                                         TO anon, authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.pt_member_profile(text, bigint, text)                                                               TO anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.dashboard_approve(text, boolean)                                                                    TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.dashboard_users_list()                                                                              TO anon, authenticated, service_role;

-- PUBLIC(=X) 엔트리에 대하여:
--   위 함수들은 pt_coach_set_login 하나만 빼고 전부 PUBLIC 에도 EXECUTE 가 있다
--   (Supabase 기본값). pt_coach_set_login 은 누군가 수동으로 REVOKE PUBLIC 한
--   흔적이며 anon/authenticated/service_role 개별 GRANT 만 남아 있다. 기능 차이는 없다.
...
--      두고 브라우저로 내려보내지 말 것.
-- 2. dashboard_approve / dashboard_users_list 는 **아무 인증도 요구하지 않는다.**
--      p_secret 파라미터가 없고 anon 에게 EXECUTE 가 열려 있다. 즉 anon 키만
--      알면 계정 목록을 읽고 가입 승인을 할 수 있다. 서버(main.ts)에서 관리자
--      세션을 확인하는 것이 유일한 방어다. 여기에 p_secret 검사를 넣는 것을
--      다음 마이그레이션 후보로 둘 것. (오늘 5건에는 포함되지 않았다)
-- 3. pt_coach_login / pt_coach_signup 은 p_secret 없이 anon 이 호출한다.
--      pt_coach_signup 은 시간당 10건 rate limit, pt_coach_login 은 5회 실패 시
--      10분 잠금이 유일한 방어다. 지점 구분 없는 전역 카운터임에 유의.
-- 4. service_role EXECUTE 가 없는 RPC 가 여전히 있다:
--      pt_consult_notified / pt_consult_purge / pt_datacenter / pt_list /
--      pt_member_profile / pt_member_purge / pt_tg_* 6종.
--      서버가 service_role 키로 이 RPC 를 호출하려 하면 실패한다.
--      (현재는 anon 키로 호출 중이라 문제가 없는 것)
-- 5. pt_passes / pt_sessions 는 RLS ON + 정책 0 + anon/authenticated 권한 0.
--      153OS CRM 화면이 이 테이블을 직접 조회한다면 항상 빈 결과다.
--      실제로 미사용 테이블일 가능성이 높다(README 참고).
-- 6. dashboard_users 는 service_role 에 SELECT 조차 없다. 워커에서 이 테이블을
