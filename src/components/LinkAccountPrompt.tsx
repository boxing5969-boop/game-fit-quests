...
const LinkAccountPrompt = () => {
  ...
  if (!needsLink || dismissed) return null;
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-5 shadow-elev-3">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-foreground">기존 회원 연동</h2>
        </div>
        <p className="mb-3 text-xs leading-relaxed text-muted-foreground">
          153복싱짐에 등록된 회원이시면, 가입하신 <b className="text-foreground">전화번호</b>를 입력해주세요.
          기존 지점·수강권 정보가 이 계정에 연동됩니다.
        </p>
        <input
          type="tel"
          value={phone}
