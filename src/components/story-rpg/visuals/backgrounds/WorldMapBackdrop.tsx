...
function MasterPathLayers({
  ...
  return (
    <>
      ...
      {/* 도시 빌딩 + 깜빡이는 창 + 등불 glow */}
      <FadeInLayer delay={0.1}>
        <BackgroundParallax mouse={mouse} depth={0.8}>
          <svg
            viewBox="0 0 800 600"
            preserveAspectRatio="xMidYMax slice"
            className="h-full w-full"
          >
            ...
            {/* 등불 광원 (빌딩 옆 amber glow) */}
            <g>
              <circle cx="180" cy="510" r="40" fill="url(#lantern-glow)" />
              <circle cx="500" cy="490" r="50" fill="url(#lantern-glow)" />
              <circle cx="730" cy="520" r="36" fill="url(#lantern-glow)" />
            </g>
            <g fill={accent}>
      ...
      {/* 안개 */}
      <FadeInLayer delay={0.2}>
        <BackgroundParallax mouse={mouse} depth={1.2}>
          <svg
            viewBox="0 0 800 600"
      ...
      <ParticleField kind="firefly" density="high" speed="slow" />
      <ParticleField kind="lantern" density="medium" speed="slow" />
    </>
  );
}
...
function ProPathLayers({
  ...
  return (
    <>
      ...
      <FadeInLayer delay={0}>
        <BackgroundParallax mouse={mouse} depth={0.4}>
          ...
          >
            ...
            {/* 광선 — 5개 ray 30° 간격 */}
            <g stroke="#fdb85c" strokeWidth="3" opacity="0.45" strokeLinecap="round">
              {Array.from({ length: 5 }).map((_, i) => {
                const angle = -60 + i * 30;
                const rad = (angle * Math.PI) / 180;
                const x2 = 400 + Math.cos(rad) * 320;
                const y2 = 490 + Math.sin(rad) * 320;
                return (
                  <line key={i} x1="400" y1="490" x2={x2} y2={y2}>
                    <animate
                      attributeName="opacity"
      ...
      <ParticleField kind="dust" density="high" speed="normal" />
    </>
  );
}
      ...
      <FadeInLayer delay={0}>
        <BackgroundParallax mouse={mouse} depth={0.4}>
          <svg viewBox="0 0 800 600" className="h-full w-full">
            <path
              d="M 200 0 L 230 220 L 200 240 L 250 480"
              stroke="#fff"
              strokeWidth="3"
              fill="none"
              opacity="0"
            ...
            <path
              d="M 600 0 L 575 260 L 625 285 L 595 510"
              stroke="#fff"
              strokeWidth="3"
              fill="none"
              opacity="0"
            ...
            <path
              d="M 400 0 L 410 200 L 380 220 L 420 460"
              stroke="#fff"
              strokeWidth="2"
              fill="none"
              opacity="0"
...
// ──────────────────────────────────────────────────────────────────
// FadeInLayer — 각 layer 진입 시 0.5s 페이드 / x slide
// ──────────────────────────────────────────────────────────────────
