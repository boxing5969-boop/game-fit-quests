  ...
  try {
    ...
    if (targetBranch && isSuperAdmin) {
      ...
      if (!profile?.branch_name) {
        console.error("[qr-token-refresh] No branch for user:", userId);
    ...
    console.log(`[qr-token-refresh] Token created for ${branchName} by ${userId} (${roleData.role}), expires in ${expirySeconds}s`);
